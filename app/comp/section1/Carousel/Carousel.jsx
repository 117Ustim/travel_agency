...
export default function Carousel() {
    ...
    const markActiveFirstVisible = () => {
        ...
        for (const el of cards) {
            ...
            if (r.right > trackRect.left + 15) { // учёт левого внутреннего отступа
                firstVisible = el;
                break;
            }
        }
        ...
        <div className={styles.carousel}>
                    ...
                    <div className={styles.card} key={`${item.id}-${idx}`}>
                        <div className={styles.imageWrap}>
                            <Image
                                ...
                                loading={idx >= items.length && idx < items.length + 3 ? undefined : "lazy"}
                                quality={75}
                                placeholder="blur"
                        ...
                        <div className={styles.cardBottom}>
                            <div className={styles.leftBlock}>
                                ...
                                <div className={styles.price}><span className={styles.euro}>€</span>{item.price}</div>
                            </div>
                            <div className={styles.rightBlock}>
                                <div className={styles.ratingValue}>{item.rating.toFixed(1)}</div>
                                <div className={styles.rhombusCol}>
                                    {Array.from({ length: 5 }).map((_, rIdx) => (
                                        <img src="/comp_1/carousel/rating.png" alt="rating" className={styles.ratingIcon} key={rIdx} />
                                    ))}
                                </div>
                            </div>
                        </div>
            ...
            <button className={styles.navNext} type="button" aria-label="Next" onClick={handleNext}>
                <span className={styles.navNextInner}>
                    ...
                    <svg width="20" height="20" viewBox="0 0 24 24" className={styles.arrowSvg} aria-hidden>
